    ...
    stratData.forEach(pos => {
        if (pos.position != activeStratData.lastTrigger) {
            if (pos.requiredConditions == 'one') {
                posStates[pos.position] = pos.conditions.some(cond => activeConditionKeys.includes(cond.name) && activeConditions[cond.name] == cond.val);
            }
            else if (pos.requiredConditions == 'all') {
                posStates[pos.position] = pos.conditions.every(cond => activeConditionKeys.includes(cond.name) && activeConditions[cond.name] == cond.val);
            }
            ;
        }
    });
